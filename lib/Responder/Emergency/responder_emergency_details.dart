...
class ResponderEmergencyDetails extends StatefulWidget {
  ...
  @override
  State<ResponderEmergencyDetails> createState() => _ResponderEmergencyDetailsState();
}
...
class _ResponderEmergencyDetailsState extends State<ResponderEmergencyDetails> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_){
      Provider.of<ResponderProvider>(context, listen: false).getEmergencyReportById(context, widget.emergencyData['emergency_id']);
    });
  }
        ...
        return Consumer<ReportProvider>(
          builder: (context, report, _) {
            return Scaffold(
              ...
              body: report.isLoading ? LoadingPage(): Stack(
                children: [
                          ...
                          CustomButton(
                            ...
                            onTap: () {
                              AppNavigationHelper.navigateToWidget(
                                  context, RespondToEmergency(reporterData: responderProvider.emergencyRes.emergency!));
                            },
...
class _EmergencyDetailsTabState extends State<EmergencyDetailsTab>
  ...
  Widget build(BuildContext context) {
    return Consumer<ResponderProvider>(
      builder: (context, responderPro, _) {
        return Column(
          children: [
            TabBar(
              dividerHeight: 0,
      ...
      builder: (context, responderPRo, _) {
        return Column(
          ...
          children: [
            ...
            AppSpaces.height8,
             Text(
 "${responderPRo.emergencyRes.emergency?.description}",              style: TextStyle(fontSize: 12, fontWeight: FontWeight.w400),
            ),
            AppSpaces.height20,
            textHeader("Victim Location"),
            AppSpaces.height8,
             MyLocationAndDestinationCard(source:  "${responderPRo.emergencyRes.emergency?.locationName}", ),
            const SizedBox(height: 16),
            textHeader("Severity level"),
            AppSpaces.height4,
            LinearProgressIndicator(
              value: responderPRo.emergencyRes.emergency?.severity == "HIGH"? 1.0 : 0.4 ,
              color: responderPRo.emergencyRes.emergency?.severity == "HIGH"? AppColors.RED: AppColors.GREEN,
              minHeight:  6,
            ...
            SingleChildScrollView(
              ...
              child: Row(
                children: List.generate(responderPRo.emergencyRes.emergency?.photos?.length ?? 0, (index) {
                  return Padding(
                    ...
                    child: SizedBox(
                      ...
                      child: ClipRRect(
                        borderRadius: BorderRadius.circular(10),
                        child: CachedNetworkImage(imageUrl:responderPRo.emergencyRes.emergency?.photos?[index] ??""),
                      ),
                    ),
                  );
                }),
