...
class ResponderCreatePasswordPage extends StatefulWidget {
  final ResponderRequest userRequest;

  const ResponderCreatePasswordPage({super.key, required this.userRequest});
  @override
  _ResponderCreatePasswordPageState createState() =>
      _ResponderCreatePasswordPageState();
}

class _ResponderCreatePasswordPageState
    extends State<ResponderCreatePasswordPage> {
  String password = '';
      ...
      body: SafeArea(
        child: Padding(
          ...
          child: Consumer<AuthProvider>(builder: (context, auth, _) {
            return auth.loadPage ? const LoadingPage(): SingleChildScrollView(
              child: Column(
                          ...
                          if (getPasswordStrength() == 1.0) {
                            if (passwordTextController.text ==
                              ...
                              widget.userRequest.password =
                                  ...
                                  print( widget.userRequest.toJson());
              
                              auth
                                  .signUp(context, widget.userRequest)
                                  .then((val) {
                                if (val == true) {
                                  alertDialog(
                                      ...
                                      message: "Done",
                                      isSuccess: true);
                                      AppNavigationHelper.setRootOldWidget(context, ResponderLoginPage());
                                }
                              });
                            } else {
