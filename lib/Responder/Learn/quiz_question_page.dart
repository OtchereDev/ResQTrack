...
class _QuizPageState extends State<QuizPage> {
  ...
  void _nextPage() {
    ...
    if (_currentPage < pro.questionResponse!.data!.quiz!.questions!.length - 1) {
      setState(() {
        _currentPage++;
      });
      ...
      body: SafeArea(
        child: Consumer<ResponderProvider>(
          builder: (context, provider, _) {
            ...
            return provider.isLoading ? const LoadingPage(): Padding(
                   ...
                   Row(
                    ...
                    children: List.generate(qust?.length ??0, (index){
                      return Padding(
                        ...
                        child: Container(
                                    height: 5,
                                    width: (Utils.screenWidth(context) / qust!.length)- 20 ,
                                    decoration: BoxDecoration(
                                      color: _currentPage == index ?  AppColors.PRIMARY_COLOR : const Color(0xffD0D5DD),
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      // if (_currentPage < _questions.length - 1)
                        Expanded(
                          child: CustomButton(
                            onTap: _currentPage <qust!.length - 1 ? _nextPage : ()=>AppNavigationHelper.navigateToWidget(context, const CompletedQuizPage()),
                            title: ('Next'),
                          ),
                        ),
                    ],
  ...
  Widget _buildQuestionPage(int index) {
    ...
    final question = responder.questionResponse!.data?.quiz?.questions?[index];
            ...
            children: [
              ...
              ...List.generate(question.options?.length??0, (optionIndex) {
                return RadioListTile(
                  ...
                  title: Text( "${question.options?[optionIndex].symbol} - ${question.options?[optionIndex].answer}"),
                  value: optionIndex,
                  groupValue: question.selectedOption,
                  onChanged: (value) {
                    responder.setAnswer(Answer(
                      answer: question.options?[optionIndex].answer,
